...
exports.copyHTML = copyHTML;
  ...
  gulp.src("dist/css/styles.css")
    ...
    .pipe(gulp.dest('dist/css')); // cwebba/style.css
    done();
  }
...
exports.imgTask = imgTask;
  ...
  /**************** jsTask task ****************/
  /** This Task is ported from Gulp_2022 (WordPress Webblog) and is overkill. all.js includes vanilla-lightbox_08.js, which produces big black SVG blots if there is no lightbox on the page. */
  function jsTask() {
    return src(jsPath)
...
function watchTask() {
    gulp.watch([imgPath], gulp.series(imgTask, reload));
    ...
    gulp.watch([jsPath], gulp.series(scripts, jsTask, reload));
    gulp.watch([hbsPath], gulp.series(hbs, reload));
    gulp.watch([htmlPath], gulp.series(copyHTML, reload));
   };
  
  //series(parallel(hbs, scripts, jsTask), scssTask, copySCSS, cssTask, copyHTML)
        
    exports.default = series( parallel(scssTask, hbs, scripts, jsTask, imgTask, copyHTML), series(copySCSS, cssTask), parallel(browser_sync, watchTask));
